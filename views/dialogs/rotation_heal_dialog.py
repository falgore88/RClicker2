...
from ui.dialogs.RotationHealDialog import Ui_Dialog


class RotationHealDialog(Ui_Dialog):
    def __init__(self, profile, parent=None, rotation=None):
        self.profile = profile
        self.rotation = rotation or Rotation.create(self.profile, Rotation.TYPE_HEAL)
    ...
    def setupUi(self, dialog):
        super(RotationHealDialog, self).setupUi(dialog)
        ...
        self.addonCoordsButton.setText(",".join(map(str, self.rotation.addon_coords)) or 'Нажмите')
        self.addonCoordsButton.clicked.connect(self.on_set_addon_coords(self.addonCoordsButton))
    ...
    def on_set_ability_coors(self, btn, color_item):
        def wrap():
            with mouse.Listener(
                    on_click=self.on_set_ability_coords_release(btn, color_item)
            ) as listener:
                listener.join()

        return wrap

    def on_set_addon_coords(self, button):
        def wrap():
            self._start_mouse_bind_thread(self.on_set_addon_coords_release(button))
    ...
    def on_set_addon_coords_release(self, btn):
    ...
    def on_save(self):
        ...
        addon_coords = self.addonCoordsButton.text()
        if addon_coords == 'Нажмите':
            addon_coords = None
        ...
        if not all((name, ability_timeout, global_timeout, rotation_bind, abilities, addon_coords)):
            ...
            self.rotation.set_addon_coords(list(map(int, addon_coords.split(','))))
